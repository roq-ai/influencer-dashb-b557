...
import { deleteInfluencerById } from 'apiSdk/influencers';
...
function CompanyViewPage() {
  ...
  const influencerHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('influencer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/influencers/create?company_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.influencer?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/influencers/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
                            <Td>{record.genre}</Td>
                            ...
                            <Td>
                              {hasAccess('influencer', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/influencers/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('influencer', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    influencerHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
